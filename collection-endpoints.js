...
class Endpoints {
    ...
    parse(path, url) {
        _.each(path, (def, /** @type {string} */method) => {
            ...
            let endpoint = new Endpoint(method, url, def, swaggerCommons);
            if (endpoint) this.endpoints.push(endpoint);
        });
    }

    async export(onExportEndpoint) {
        let collection = new ps.Collection({
            info: {
        ...
        if (this.swagger.securityDefinitions) {
            ...
            const securityValues = this.globalVariables.filter( obj => {
                return obj.name == 'client_id' || obj.name == 'client_secret' || obj.name == 'username' || obj.name == 'password'
             }) || [];
        ...
        this.endpoints.forEach(endpoint => { 
            ...
            if (this.swagger.securityDefinitions && endpoint.def.security) {
                ...
                if (security.variable && !this.globalVariables.some(v => v.name === security.variable.name)) this.globalVariables.push(security.variable);
            }

            let members = endpoint.export(this.swagger['x-pm-test'], security);     
            let folder;   
            if (endpoint.def.tags && endpoint.def.tags.length > 0) {
                folder = _.find(collection.items.members, e => e.name == endpoint.def.tags[0]);
            ...
            // Test definition
            definitionTestsPassed = deinitionTests.definitionTestsPassed(endpoint, this.swagger.security) && definitionTestsPassed;
        });
        ...
        this.globalVariables.forEach(variable => {
            ...
            {
                collection.variables.add({ key: 'base-url', id: 'base-url', value: variable.value + this.swagger.basePath, type: 'string' });
                return;
            }
        ...
        {
            collection.variables.add({ key: 'base-url', id: 'base-url', value: this.swagger.schemes[0] + '://' + this.swagger.host + this.swagger.basePath, type: 'string' });
        } 
...
async function getSecurities(securityDef, tokenUrl) {
    ...
    if (securityDef.hasOwnProperty('oauth2') && securityDef.oauth2.flow == 'password') {
        const b = new Buffer(securityDef.value.client_id +':'+ securityDef.value.client_secret);
